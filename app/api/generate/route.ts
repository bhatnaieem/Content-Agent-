...
function parseExcludedCookie(request:Request):string[]{
  const raw=request.headers.get("cookie")||"";
  const match=raw.match(/(?:^|;\s*)web3pulse_excluded=([^;]+)/);
  if(!match)return [];
  try{const parsed=JSON.parse(decodeURIComponent(match[1]));return Array.isArray(parsed)?parsed.filter(id=>typeof id==="string").slice(-50):[];}catch{return [];}
}
...
function normalizeStory(story:any,candidates:FreshCandidate[]):Story|null{
...
export async function POST(request:Request){
  ...
  try{
    ...
    const cookieExcluded=parseExcludedCookie(request);
    const requestedExcluded=Array.isArray(body.excludeCandidateIds)?body.excludeCandidateIds.filter(id=>typeof id==="string"&&id.trim()):[];
    const excludedIds=Array.from(new Set([...cookieExcluded,...requestedExcluded])).slice(-50);
    ...
    if(!freshCandidates.length)return NextResponse.json({error:excludedIds.length?"No new verified stories are available in the current research window. Web3 Pulse will not repeat a previously generated story.":"No fresh verified candidates were selected. Web3 Pulse will not generate stale content.",code:excludedIds.length?"NO_NEW_STORIES":"NO_FRESH_CANDIDATES",date:currentDate,cutoff_utc:cutoffUtc,excluded_count:excludedIds.length,agents:packet.agents,verification:packet.verification},{status:503});
    ...
    const newlyUsed=freshStories.flatMap(s=>s.candidate_ids);response.cookies.set("web3pulse_excluded",encodeURIComponent(JSON.stringify(Array.from(new Set([...excludedIds,...newlyUsed])).slice(-50))),{httpOnly:true,sameSite:"lax",secure:true,path:"/",maxAge:60*60*24*7});
    return response;
