...
Find the TWO strongest recent Web3 opportunities from the latest 48 hours and return strict JSON.
...
Prefer genuinely recent, high-impact stories from the latest 48 hours. For every story, provide source details ONLY when actually known. NEVER invent a URL, source name, or publication date.
...
Prefer primary/official sources, security researchers, major crypto publications, regulatory filings, company announcements, and reputable market data.
...
function normalizeStory(story: any): Story { const sourceDetails = cleanSourceDetails(story?.source_details); const legacySources = Array.isArray(story?.sources) ? story.sources.filter((item: unknown) => typeof item === "string") : []; return { ...story, sources: legacySources.length ? legacySources : sourceDetails.map((item) => item.url || item.name).filter(Boolean), source_details: sourceDetails }; }
...
export async function POST(request: Request) {
  try {
    ...
    const research = await generateWithLLM({ provider, system: RESEARCH_PROMPT, user: "Research the latest high-interest Web3 opportunities. Strongly prioritize airdrops, actionable airdrop guides, exploits and hacks over generic crypto news. Return strict JSON." + clientContext(body.clientProfile), responseFormat: "json_object", temperature: 0.2 });
    const data = JSON.parse(research.content);
    const stories: Story[] = Array.isArray(data.stories) ? data.stories.map(normalizeStory) : [];
    const enriched = await Promise.all(stories.map(async (story) => { try { const formats = await generateWithLLM({ provider, system: FORMAT_PROMPT, user: ["Create the six distinct Content Studio assets for this story.", `Headline: ${story.headline}`, `Category: ${story.category}`, `Summary: ${story.summary}`, `Reason: ${story.reason}`, `Sources: ${(story.sources || []).join(" | ")}`, `Keywords: ${(story.keywords || []).join(", ")}`, `Core thread: ${(story.thread?.tweets || []).join("\n")}`, clientContext(body.clientProfile)].join("\n"), responseFormat: "json_object", temperature: 0.65 }); const assets = JSON.parse(formats.content); return { ...story, graphic_prompt: assets.creative || story.graphic_prompt, alt_text: assets.alt_text || story.alt_text, engagement: { reply: assets.reply || "", quote_tweet: assets.quote_tweet || "", poll: assets.poll || "", blog_expansion: assets.blog_expansion || "" } }; } catch (error) { console.error("Content format generation failed:", error); return { ...story, engagement: { reply: "Add a specific reaction or question to this story.", quote_tweet: `This development deserves a closer look: ${story.headline}`, poll: "Question: What matters most here?\nOptions: A) Adoption\nB) Innovation\nC) Regulation\nD) Market impact", blog_expansion: [story.headline, "", "Introduction", story.summary, "", "Key implications", story.reason, "", "Conclusion", story.cta].join("\n\n") } }; }));
    return NextResponse.json({ ...data, stories: enriched, llm_provider: research.provider, llm_model: research.model, client_mode: Boolean(body.clientProfile), client_name: body.clientProfile?.name || null, priority_focus: ["Exploits & Hacks", "Airdrops", "Airdrop Guides"] });
