...
function resolveCandidateIds(story: any, candidates: FreshCandidate[]): string[] {
  ...
  const ids = explicit.filter((id: unknown): id is string => typeof id === "string" && byId.has(id));
  if (ids.length) return Array.from(new Set(ids));
  const suppliedUrls = new Set<string>();
  if (Array.isArray(story?.sources)) for (const url of story.sources) if (typeof url === "string") suppliedUrls.add(normalizeUrl(url));
  if (Array.isArray(story?.source_details)) for (const detail of story.source_details) if (typeof detail?.url === "string") suppliedUrls.add(normalizeUrl(detail.url));
  const urlMatches = candidates.filter(c => suppliedUrls.has(normalizeUrl(c.url))).map(c => c.id);
