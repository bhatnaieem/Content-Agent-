...
// Keep the anti-repeat window short enough to prevent stale history from exhausting current briefings.
// Exact URL/headline repeats are blocked during this window; rejected drafts never block anything.
...
function isProtectedHistory(row: HistoryRow) { const status = String(row.status || "generated").toLowerCase(); if (status === "rejected") return false; const generated = Date.parse(row.generated_at); return Number.isFinite(generated) && generated >= Date.now() - DUPLICATE_COOLDOWN_HOURS * 60 * 60 * 1000; }
