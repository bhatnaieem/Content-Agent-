...
function headlineTokens(value: string) { const stop = new Set(["the","a","an","and","or","of","to","from","in","on","for","with","across","after","before","as","at","by","is","are","was","were","this","that","crypto","web3","report","reports","says","saying","new","latest","today","update","updates"]); return new Set(normalizeHeadline(value).split(" ").filter(token => token.length >= 3 && !stop.has(token))); }
export function headlineSimilarity(a: string, b: string) { const aa = headlineTokens(a), bb = headlineTokens(b); if (!aa.size || !bb.size) return 0; let overlap = 0; for (const token of aa) if (bb.has(token)) overlap++; const jaccard = overlap / (aa.size + bb.size - overlap); return overlap >= 4 && jaccard >= 0.62 ? jaccard : 0; }
...
// Candidate IDs are feed identifiers, not durable event identities. Never use them alone
// to reject a newly discovered candidate. Durable duplicate protection is URL + event similarity.
...
export async function filterGeneratedDuplicates<T extends { headline: string; candidate_ids: string[]; sources: string[] }>(stories: T[]) {
  ...
  for (const story of stories) {
    ...
    const duplicate = !id || batchIds.has(id) ||
      urls.some(url => usedUrls.has(normalizeHeadline(url)) || batchUrls.has(normalizeHeadline(url))) ||
      usedHeadlines.some(h => headlineSimilarity(story.headline, h) > 0) ||
      batchHeadlines.some(h => headlineSimilarity(story.headline, h) > 0);
