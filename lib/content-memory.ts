...
const DUPLICATE_COOLDOWN_DAYS = 14;
...
function isProtectedHistory(row: HistoryRow) { const status = String(row.status || "generated").toLowerCase(); if (status === "rejected") return false; const generated = Date.parse(row.generated_at); return Number.isFinite(generated) && generated >= Date.now() - DUPLICATE_COOLDOWN_DAYS * 24 * 60 * 60 * 1000; }
...
// A story is protected from repetition during a practical editorial cooldown, not forever.
// This prevents old history or rejected drafts from exhausting the live research pool.
export async function filterPreviouslyCovered<T extends { id: string; title: string; url: string }>(candidates: T[]) {
  if (!candidates.length) return candidates;
  const history = (await getRecentHistory()).filter(isProtectedHistory);
  ...
  return candidates.filter(candidate => !usedUrls.has(normalizeHeadline(candidate.url)) && !usedHeadlines.has(normalizeHeadline(candidate.title)));
}
