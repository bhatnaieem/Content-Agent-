...
// Duplicate protection is long-lived. A previously covered event must not return merely because a short window expired.
...
// Hard gate: used ID, used source, or materially similar headline/event = ineligible forever in editorial memory.
export async function filterPreviouslyCovered<T extends { id: string; title: string; url: string }>(candidates: T[]) {
  ...
  return candidates.filter(candidate => {
    ...
    if (usedHeadlines.some(headline => headlineSimilarity(candidate.title, headline) >= 0.65)) return false;
    return true;
  });
}
...
export async function saveGeneratedStories(stories: MemoryStory[]) {
  ...
  for (const story of stories) {
    ...
    if (seenHeadlines.some(existing => headlineSimilarity(existing, headline) >= 0.65)) continue;
